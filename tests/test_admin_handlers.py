...
with patch.dict(sys.modules, {"database": fake_db_module}):
    ...
    from utils import STATE_ADMIN_GIVE_DAYS, STATE_ADMIN_GIVE_USER_ID, STATE_BAN_REASON, STATE_FIND_USER, STATE_SIMULATE_REFERRAL_USERID
...
class AdminHandlerTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_handle_admin_give_subscription_sets_state(self):
        with patch.object(admin_handlers, "build_admin_panel", return_value=("panel", None)):
    ...
    async def test_handle_admin_give_tariff_requires_target_user(self):
        await admin_handlers.handle_admin_give_tariff(self.update, self.context, 1, "basic")

        self.assertIn("ID пользователя не найден", self.update.callback_query.edited_messages[0]["text"])
    ...
    async def test_handle_admin_give_tariff_sets_admin_state(self):
        ...
        self.assertIn("Введите количество дней", self.update.callback_query.edited_messages[0]["text"])
    ...
    async def test_handle_admin_simulate_referral_sets_state(self):
        await admin_handlers.handle_admin_simulate_referral(self.update, self.context, 1)

        self.assertEqual(self.context.user_data["state"], STATE_SIMULATE_REFERRAL_USERID)
    ...
    async def test_handle_unban_logs_and_updates(self):
        with patch.object(admin_handlers.db, "unban_user") as unban_mock, \
    ...
    async def test_handle_makeadmin_logs_and_updates(self):
        with patch.object(admin_handlers.db, "set_admin") as set_admin_mock, \
             patch.object(admin_handlers.db, "log_admin_action") as log_mock:
    ...
    async def test_handle_removeadmin_logs_and_updates(self):
        with patch.object(admin_handlers.db, "remove_admin") as remove_admin_mock, \
             patch.object(admin_handlers.db, "log_admin_action") as log_mock:
