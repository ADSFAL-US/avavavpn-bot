...
class XControllerClientTests(unittest.TestCase):
    def test_make_request_returns_json_for_success(self):
        client = xcontroller_client.XControllerClient(
    ...
    def test_make_request_raises_auth_error(self):
        ...
        with (
            patch.object(
                client._session,
                "request",
                side_effect=xcontroller_client.XControllerAuthError(
                    "Invalid credentials"
                ),
            ),
            self.assertRaises(xcontroller_client.XControllerAuthError),
        ):
            client._make_request("GET", "/api/health")

    def test_get_subscription_link_uses_base_url(self):
        client = xcontroller_client.XControllerClient(
            base_url="https://panel.test", username="u", password="p"
        )
        self.assertEqual(
            client.get_subscription_link("abc"), "https://panel.test/sub/abc"
        )
    ...
    def test_create_user_subscription_uses_tariff_defaults(self):
        ...
        with patch.object(
            client, "create_subscription", return_value={"success": True}
        ) as create_mock:
            result = client.create_user_subscription(
                123, {"preset_id": 3, "duration_days": 10, "traffic_limit_gb": 20}
            )
    ...
    def test_get_panel_details_returns_none_for_not_found(self):
        ...
        with patch.object(
            client,
            "_make_request",
            side_effect=xcontroller_client.XControllerAPIError("missing", 404, {}),
        ):
