...
import config
from database_compat import db, TARIFFS
from yookassa import YooKassaAPI
...
# Payment storage - will be initialized after DB is ready
...
async def text_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == STATE_FIND_USER:
        ...
        if not test_user.get("referred_by") or test_user["referred_by"] != admin_id:
            ...
            cursor.execute(
                "UPDATE users SET has_rewarded_referrer = 0, has_used_discount = 0 WHERE user_id = ?", (test_user_id,))
            db.conn.commit()
...
async def on_startup(app):
    ...
    await init_db()
    # Initialize payment storage with new async DB
    from db.engine import async_session_maker
    from db.repositories.payment_repo import PaymentRepository

    # Create a wrapper for PaymentStorage that uses async repos
    class AsyncPaymentStorage:
        def __init__(self):
            self._session_maker = async_session_maker

        def create_payment_record(self, order_id, user_id, tariff_id, amount, payment_id=None, metadata=None):
            # This is called from sync context, need to run async
            import asyncio

            async def _create():
                async with self._session_maker() as session:
                    repo = PaymentRepository(session)
                    await repo.create(order_id, user_id, tariff_id, amount, payment_id, metadata)
        ...
        def update_payment_status(self, order_id, status, payment_id=None):
            ...
            async def _update():
                async with self._session_maker() as session:
                    repo = PaymentRepository(session)
                    if status == "completed":
                        return await repo.mark_completed(order_id, payment_id)
                    else:
                        # For other statuses, we need a different method
        ...
        def get_pending_payments(self, user_id):
            ...
            async def _get():
                async with self._session_maker() as session:
                    repo = PaymentRepository(session)
                    payments = await repo.get_pending_for_user(user_id)
                    return [
                        {
                            "id": p.id,
                            "order_id": p.order_id,
        ...
        def clean_old_payments(self, hours=24):
            import asyncio

            async def _clean():
                async with self._session_maker() as session:
                    repo = PaymentRepository(session)
                    return await repo.clean_old_payments(hours)
            return asyncio.run(_clean())

    app_context.payment_storage = AsyncPaymentStorage()
    logger.info("Database and payment storage initialized")
