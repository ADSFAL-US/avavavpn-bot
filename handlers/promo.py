...
from utils import btn, back_btn, STATE_PROMO_CODE
import app_context

logger = logging.getLogger(__name__)


async def handle_promo_activate(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
    ...
    if context.user_data.get("state") == STATE_PROMO_CODE:
        ...
        text = (
            f"✅ <b>{result.get('message', 'Промокод успешно активирован')}</b>\n\n"
        )
...
async def handle_promo_menu(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
    ...
    keyboard = [
        [btn("📝 Ввести промокод", "promo_activate")],
        [back_btn("main_menu")]
    ]
